...
CountVis.prototype.initVis = function () {
    ...
    self.yScale = d3.scale.pow().exponent(0.5).range([self.graphH, 0]); // POWER SCALE HERE !!!
    ...
    function brushed(){
 	//svg = d3.select("#countVis svg");
 	//var brush = d3.select("#countVis svg g");
 	//console.log(brush)
 	var extent = (brush.empty()? null : brush.extent());
 	//console.log(self);
 	if(extent){
    ...
    //var visScale = d3.time.scale().domain([new Date("2014/12/3"), new Date("2014/12/12")]).range([0,500])
    	 brush = d3.svg.brush().x(self.xScale).on("brushend", brushed);
    
    // create the brush, and
    self.svg.select("g").append("g").attr("class", "brush").call(brush)
  .selectAll("rect").attr({
    height: self.graphH
});
...
CountVis.prototype.addSlider = function (svg) {
	...
	//var svg= d3.select("#countVis svg");
